...
// @author Adapted from Jef Poskanzer's Java port by way of J.M.G. Elliott. K Weiner 12/00
...
class LZWEncoder {
    ...
    // way to a faster exclusive-or manipulation. Also do block compression with an adaptive reset,
    // whereby the code table is cleared when the compression ratio decreases, but after the table
    // fills. The variable-length output codes are re-sized at this point, and a special CLEAR code
    ...
    // Inputs:
    // code: A n_bits-bit integer. If == -1, then EOF. This assumes that n_bits =< wordsize - 1.
    // Outputs:
    ...
    /**
     * Add a character to the end of the current packet, and if it is 254 characters, flush the
     * packet to disk.
    ...
    /**
     * Compress the image
     * @param init_bits - initial bits of the image
    ...
    /**
     * Output the buffer to the output stream
     * @param code - code of output
